<template>
  <div id="highlight" class="highlight border-t-1 border-grey">
        <div class="m-16">
            <div class="flex justify-center w-full text-3xl m-6" v-in-viewport.once="startAnimation=true">
                Recent blog entries
            </div>
                    <transition-group name="fade" tag="div" class="flex mb-4 flex-wrap">
                        <div class="w-1/2 my-4" v-for="post in blogEntries" :key="post.id" v-show="startAnimation">
...
<script>
import blogEntriess from '../serverdata'
...
export default {
  ...
  created () {
    setTimeout(() => { this.blogEntries = blogEntriess }, 1000)
    // this.blogEntries = blogEntriess
  }
}
</script>
